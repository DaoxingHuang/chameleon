...
export async function createMaterialFromGLTF(
  ...
  if (!materialInfo) return undefined;

  const ext = materialInfo.extensions && materialInfo.extensions.ANT_materials_shader;
  if (!ext) return undefined;

  const gltfExt = (gltf && gltf.extensions && (gltf.extensions as any).ANT_materials_shader) || null;
  const shaderIndex = ext.shader;
  const shaderEntry = gltfExt && Number.isFinite(shaderIndex) ? gltfExt.shaders && gltfExt.shaders[shaderIndex] : null;
  ...
  // Merge shaderDef.properties (top-level) with material-level ext.properties
  const topProps = (shaderDef && shaderDef.properties) || {};
  const materialProps = ext && ext.properties ? ext.properties : {};
  const mergedProps: Record<string, any> = {};
  for (const k of Object.keys(topProps)) mergedProps[k] = (topProps as any)[k];
  for (const k of Object.keys(materialProps)) mergedProps[k] = (materialProps as any)[k];
